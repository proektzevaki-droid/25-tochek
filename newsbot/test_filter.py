...
print("\n6. Украина Online — впереди гнев или скорбь")
uaonline = pattern_filter("🤬|😢|😭|🙏")
...
check(
    "впереди встречаются 🤬, 😢 и 🙏",
    sorted({pairs[0][0] for pairs in UAONLINE_OK}),
    sorted({"🤬", "😢", "🙏"}),
)
check(
    "вторая реакция любая — от 😁 до ❤️",
    len({pairs[1][0] for pairs in UAONLINE_OK}) >= 6,
    True,
)
passed, reason = evaluate(post(("👍", 800), ("🤬", 400)), uaonline, 0, 0)
check("🤬 только вторым → пропускаем", passed, False)
print(f"       причина: {reason}")
