...
# Одиночное слово нельзя резать по буквам, как слитную запись эмодзи.
check("одно слово = одна позиция", parse_pattern("злой")[0].emojis, frozenset({"🤬"}))
check("одно слово: длина шаблона", len(parse_pattern("злой")), 1)
...
# ─────────────────────────────────────────────────────────────────────────────
# Украина Online — правило зеркально Осташко. Там гнев и скорбь впереди
# отсекают пост, здесь ровно они и нужны. Вторая реакция роли не играет.
# ─────────────────────────────────────────────────────────────────────────────
UAONLINE_OK = [
    (("🤬", 2000), ("😁", 149), ("🙏", 130), ("❤️", 102)),
    (("🤬", 1600), ("🙏", 542), ("❤️", 198), ("🔥", 96)),
    ...
    (("🤬", 2043), ("😢", 140)),
    (("🙏", 2236), ("❤️", 227)),
    # Здесь 🤬 только на втором месте, а впереди 😁 — пост всё равно нужен.
    (("😁", 1767), ("🤬", 555)),
    (("🤬", 3584), ("🤯", 621)),
    (("🤬", 1304), ("😁", 248)),
    (("🤬", 2181), ("😁", 479)),
...
# Сочетания записаны как есть, без обобщения: ровно те пары, что Андрей
# отметил как нужные. Новая пара добавляется отдельной строкой.
UAONLINE_PAIRS = [
    "🤬, 😁",
    "🤬, 🙏",
    "🤬, 😢",
...
print("\n6. Украина Online — список отмеченных сочетаний")
uaonline = pattern_filter(*UAONLINE_PAIRS)
...
check(f"различных сочетаний в списке", len(UAONLINE_PAIRS), 10)
passed, reason = evaluate(post(("👍", 800), ("🔥", 400)), uaonline, 0, 0)
check("сочетание не из списка → пропускаем", passed, False)
print(f"       причина: {reason}")

print("\n6b. Два канала — зеркальные правила на одних и тех же постах")
# Ровно то, что делает фильтр настраиваемым по каналам, а не общим.
mirror = [(("🤬", 2043), ("😢", 140)), (("👍", 523), ("😁", 138))]
for pairs in mirror:
    in_ua = evaluate(post(*pairs), uaonline, 0, 0)[0]
    in_ost = evaluate(post(*pairs), ostashko, 0, 0)[0]
    check(f"{pairs[0][0]}{pairs[1][0]}: UaOnlii={in_ua}, Осташко={in_ost}", in_ua, not in_ost)
